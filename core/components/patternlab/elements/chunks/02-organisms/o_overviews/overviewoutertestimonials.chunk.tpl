...
<div class="ui [[+cols]] column doubling [[+row_tpl:containsnot=`logo`:then=`stackable`]] [[+padding]] nested [[+[[+prefix]].box_type]]">
    [[![[If? &subject=`[[+pagination]]` &operator=`EQ` &operand=`1` &then=`getPage@SemanticUI` &else=`getCache`]]?
        ...
        &parents=`[[If? &subject=`[[+resources_testimonials]]` &operator=`notempty` &then=`-1` &else=`[[++patternlab.testimonial_container_id]]`]]`
        &resources=`[[+resources_testimonials]]`
        &depth=`99`
        &limit=`[[+limit]]`
        ...
        &processTVs=`1`
        &showHidden=`1`
        &sortby=`[[If? &subject=`[[+resources_testimonials]]` &operator=`notempty` &then=`FIELD(modResource.id, [[+resources_testimonials]])` &else=`[[+sortby]]`]]`
