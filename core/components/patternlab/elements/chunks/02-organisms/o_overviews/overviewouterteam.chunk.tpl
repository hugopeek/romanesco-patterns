...
<div class="ui [[+cols]] column doubling stackable [[+padding]] nested [[+[[+prefix]].box_type]]">
    [[![[If? &subject=`[[+pagination]]` &operator=`EQ` &operand=`1` &then=`getPage@SemanticUI` &else=`getCache`]]?
        ...
        &parents=`[[If? &subject=`[[+resources_team]]` &operator=`notempty` &then=`-1` &else=`[[++patternlab.team_container_id]]`]]`
        &resources=`[[+resources_team]]`
        &depth=`99`
        &limit=`[[+limit]]`
        ...
        &processTVs=`1`
        &showHidden=`1`
        &sortby=`[[If? &subject=`[[+resources_team]]` &operator=`notempty` &then=`FIELD(modResource.id, [[+resources_team]])` &else=`[[+sortby]]`]]`
