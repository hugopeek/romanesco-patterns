...
[[+field_type:contains=`multiple`:then=`multiple`:toPlaceholder=`[[+name]]-multiple`]]
[[If?
    &subject=`[[+[[+name]]-multiple]][[+option_other]]`
    &operator=`eq`
    &operand=`1`
    &then=`1`
    &else=`0`
...
<div class="[[+field_width:after=` wide `]][[+field_layout:is=`inherit`:then=`[[+form_layout]]`:else=`[[+field_layout]]`]] field [[If? &subject=`[[+field_required]]` &operator=`eq` &operand=`1` &then=`required`]] [[!If? &subject=`[[+fb[[*id]].error.[[+name]]]]` &operator=`notempty` &then=`error`]]">
    ...
    <select name="[[+name]][[+[[+name]]-multiple:notempty=`[]`]]" id="[[+name]]" class="ui [[+field_type:empty=`dropdown`]] [[+option_other:is=`1`:then=`with other`]]" [[+[[+name]]-multiple]]>
