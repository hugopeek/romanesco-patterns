<div class="[[+field_layout:is=`inherit`:then=`[[+form_layout]]`:else=`[[+field_layout]]`]] file-upload field [[!If? &subject=`[[!+fb[[*id]].error.[[*id:md5]][[+field_type_idx]]]]` &operator=`notempty` &then=`error`]] [[If? &subject=`[[+field_required]]` &operator=`eq` &operand=`1` &then=`required`]]">
