...
if (is_object($object) && isset($properties['key'])) {
    $children = $modx->getCollection('FractalFarming\Romanesco\Model\Option', array('group' => $object->get('id')));

    foreach ($children as $child) {
        $child = $modx->getObject('FractalFarming\Romanesco\Model\Option', array('id' => $child->get('id')));
...
//    $q = $modx->newQuery('FractalFarming\Romanesco\Model\OptionGroup');
//    $q->select(array(
//        "max(position)",
//    ));
